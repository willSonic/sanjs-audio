 ...
 */
angular.module('sonicMasher', [])

    .directive('sonicMasher', ['$rootScope', '$log', '$interpolate', '$timeout', 'throttle',
        function ($rootScope, $log, $interpolate, $timeout, throttle) {

            /**
             * @usage: new SonicMasher(element, scope, [playlist], [options]);
             *
             ...
             */
            var SonicMasher = function (element, scope, playlist, options) {
                if (!(this instanceof SonicMasher)) { return new SonicMasher(element, scope, playlist, options); }
            ...
            SonicMasher.prototype = {
                ...
                play: function (index) {
                    ...
                    if (!this.currentTrack && this._sonicTag.readyState) { this.currentTrack++; }
                    this._sonicTag.play();
                },
                ...
                pause: function () {
                    this._sonicTag.pause();
                },
                toggleMute: function () {
                    this.muted = this._sonicTag.muted = !this._sonicTag.muted;
                },
                    ...
                    var self = this,
                        ...
                        updateTime = throttle(1000, false, function (evt) {
                            scope.$apply(function () {
                                self.currentTime = self.position = self._sonicTag.currentTime;
                        ...
                        setDuration = function (evt) {
                            scope.$apply(function () {
                                ...
                                self.duration = self._sonicTag.duration;
                                self.formatDuration = self._formatTime(self.duration);
                                self.loadPercent = parseInt((self._sonicTag.buffered.end(self._sonicTag.buffered.length - 1) / self.duration) * 100, 10);
                            });
                        },
                        ...
                        updateProgress = function (evt) {
                            if (self._sonicTag.buffered.length) {
                                scope.$apply(function () {
                                    self.loadPercent = parseInt((self._sonicTag.buffered.end(self._sonicTag.buffered.length - 1) / self.duration) * 100, 10);
                                });
                            }
            ...
            return {
                ...
                link: function (scope, element, attrs, ctrl) {
                    if (element[0].tagName !== 'AUDIO') {
                        return $log.error('sonicMasher directive works only when attached to an <audio> type tag');
                    }
                    var audioElement = [],
                    ...
                    scope.exposedPlayer = new SonicMasher(element, scope, playlist);
